...
function parseStreamInfo(data: Buffer): { sampleRate: number; totalSamples: number } {
  // Sample rate: top nibble of data[12] holds bits [3:0], data[10] and data[11] hold bits [19:4]
  const sampleRate =
    (data[10] << 12) | (data[11] << 4) | ((data[12] >> 4) & 0x0f);
...
export async function parseFlac(filePath: string): Promise<number> {
  ...
  try {
    ...
    if (blockType !== 0) {
      throw new InvalidFileError(
        `Expected STREAMINFO (type 0) as first metadata block, got type ${blockType}`
      );
    }
