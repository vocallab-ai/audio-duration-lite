...
const FIRST_READ_SIZE = 8192; // enough for the Vorbis ID header in page 1
const TAIL_READ_SIZE = 65536; // scan last 64 KiB for the final page
...
function readVorbisSampleRate(buf, pageOffset) {
    ...
    if (buf[packetOffset] !== 0x01) {
        throw new InvalidFileError(`Expected Vorbis identification packet (type 0x01), got 0x${buf[packetOffset].toString(16)}`);
    }
 ...
 */
function readGranulePosition(buf, pageOffset) {
 ...
 */
function findLastOggPage(buf) {
    for (let i = buf.length - 4; i >= 0; i--) {
        if (buf[i] === OGG_CAPTURE[0] &&
...
// ─── Public API ──────────────────────────────────────────────────────────────
export async function parseOgg(filePath) {
