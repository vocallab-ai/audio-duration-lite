 ...
 */
function scanChunks(buf, bytesRead) {
    let fmt = null;
    let dataSize = null;
    ...
    while (offset + 8 <= bytesRead) {
        ...
        if (fmt !== null && dataSize !== null)
            break;
        // Advance to next chunk; RIFF chunks must be word-aligned
        const advance = 8 + chunkSize + (chunkSize % 2 !== 0 ? 1 : 0);
        if (advance === 0)
            break; // guard against malformed zero-size chunks
        offset += advance;
    }
    if (fmt === null)
        throw new InvalidFileError("WAV file is missing fmt chunk");
    if (dataSize === null)
        throw new InvalidFileError("WAV file is missing data chunk");
    return { fmt, dataSize };
}
export async function parseWav(filePath) {
