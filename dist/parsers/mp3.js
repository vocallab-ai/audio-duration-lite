...
function decodeSyncsafe(buf, offset) {
...
function parseFrameHeader(buf, offset) {
    ...
    if (mpegVersionBits === 1)
        return null; // reserved version
    if (layerBits === 0)
        return null; // reserved layer
    if (bitrateIdx === 0 || bitrateIdx === 0x0f)
        return null; // free / bad bitrate
    if (sampleRateIdx === 0x03)
        return null; // reserved sample rate
 ...
 */
function xingPayloadOffset(frame, frameOffset) {
 ...
 */
function readXingFrameCount(buf, frame, frameOffset) {
    ...
    if (!(flags & 0x01))
        return null; // frame count field not present
 ...
 */
function readVbriFrameCount(buf, frameOffset) {
    const vbriOffset = frameOffset + 4 + 32; // always 36 bytes from frame start
    if (vbriOffset + 18 > buf.length)
        return null;
    if (buf.toString("ascii", vbriOffset, vbriOffset + 4) !== "VBRI")
        return null;
...
const HEADER_READ_SIZE = 10240;
export async function parseMp3(filePath) {
